...
void getWWScale0jet  (TString analysis = "HWW", TString WW_control_level = "09_WWControl0Jet/0jet", TString HWW_signal_level = "06_mllHighNOdphiMET/0jet", double lumi_fb = 35.9) {
 ...
 for (int i=ee; i<ll; i++)
    {
      h1_data[i] = (TH1D*)file_data ->Get(analysis + "/" + WW_control_level + "/h_counterLum_" + schannel[i]);
      h1_f   [i] = (TH1D*)file_f    ->Get(analysis + "/" + WW_control_level + "/h_counterLum_" + schannel[i]);
      h1_wz  [i] = (TH1D*)file_wz   ->Get(analysis + "/" + WW_control_level + "/h_counterLum_" + schannel[i]);
      h1_zz  [i] = (TH1D*)file_zz   ->Get(analysis + "/" + WW_control_level + "/h_counterLum_" + schannel[i]);
      h1_tt  [i] = (TH1D*)file_tt   ->Get(analysis + "/" + WW_control_level + "/h_counterLum_" + schannel[i]);
      h1_st  [i] = (TH1D*)file_st   ->Get(analysis + "/" + WW_control_level + "/h_counterLum_" + schannel[i]);
      h1_ww  [i] = (TH1D*)file_ww   ->Get(analysis + "/" + WW_control_level + "/h_counterLum_" + schannel[i]);
      h1_dy  [i] = (TH1D*)file_dy   ->Get(analysis + "/" + WW_control_level + "/h_counterLum_" + schannel[i]);
      h1_wjets[i]= (TH1D*)file_wjets->Get(analysis + "/" + WW_control_level + "/h_counterLum_" + schannel[i]);  
      h1_ttv [i] = (TH1D*)file_ttv  ->Get(analysis + "/" + WW_control_level + "/h_counterLum_" + schannel[i]);
      h1_hww [i] = (TH1D*)file_hww  ->Get(analysis + "/" + WW_control_level + "/h_counterLum_" + schannel[i]);
      h1_wg  [i] = (TH1D*)file_wg   ->Get(analysis + "/" + WW_control_level + "/h_counterLum_" + schannel[i]);
      h1_zg  [i] = (TH1D*)file_zg   ->Get(analysis + "/" + WW_control_level + "/h_counterLum_" + schannel[i]);
      h1_vvv [i] = (TH1D*)file_vvv  ->Get(analysis + "/" + WW_control_level + "/h_counterLum_" + schannel[i]);
      h1_hz  [i] = (TH1D*)file_hz   ->Get(analysis + "/" + WW_control_level + "/h_counterLum_" + schannel[i]);
      h1_wgs [i] = (TH1D*)file_wgs  ->Get(analysis + "/" + WW_control_level + "/h_counterLum_" + schannel[i]);
      ...
      mc_yield_err [i] = sqrt((h1_f[i] -> GetBinError(2)) * (h1_f[i] -> GetBinError(2)) + ((h1_wz[i] -> GetBinError(2)) * (h1_wz[i] -> GetBinError(2)) 
       ...
       h1_tt_mc_signallevel [i]  = (TH1D*)file_tt  ->Get(analysis + "/" + HWW_signal_level + "/h_counterLum_" + schannel[i]);
       ...
       WW_bkg [i]     = tt_mc_signallevel[i] * SF[i];
       WW_bkg_err [i] = sqrt(SF[i]*SF[i]*tt_mc_signallevel_err[i]*tt_mc_signallevel_err[i] + tt_mc_signallevel[i]*tt_mc_signallevel[i]*SF_err[i]*SF_err[i]);
       
       cout << "Canal " << schannel[i] << ": WW bkg = " << WW_bkg[i] << "+-" << WW_bkg_err[i] << endl;
